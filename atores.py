...
class Ator():
    ...
    def colidir(self, outro_ator, intervalo=1):
        ...
        if self.status == DESTRUIDO or outro_ator.status == DESTRUIDO:
            colodiu_na_horizontal = self.esta_dentro_do_intervalo(self.x,outro_ator.x,intervalo)
            colodiu_na_vertical = self.esta_dentro_do_intervalo(self.y,outro_ator.y,intervalo)
            if colodiu_na_horizontal and colodiu_na_vertical:
